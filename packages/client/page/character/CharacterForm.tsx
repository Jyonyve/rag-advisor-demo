...
import { isCharacterInfo, createBasicCharacterInfo } from '@rag-advisor-demo/shared/util';
import { blobToWebpFile, cleanupBlobUrl, processCroppedImage } from '../../util/cropImageUtils.js';
import { getCharacterCropAspect, type CharacterCropStage } from './characterImageCrop.js';
...
export const CharacterForm: FC<Props> = ({
	...
	const uploadPortraits = async (
		...
		await Promise.all(
			images
				.filter((img) => img.file && img.avatarFile && !img.toDelete)
				.map((image) => {
	...
	const applyPortraitDiffs = async (characterId: string): Promise<void> => {
		...
		const replacements = uploadedImages.filter((img) => img.file && img.avatarFile && !img.toDelete);
		...
		if (deletions.length > 0 || replacements.length > 0) {
			await refreshCharacterImages(characterId);
		}
	};

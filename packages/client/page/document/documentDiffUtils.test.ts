...
const documentWithRagPreference = (
	documentId: string,
	includeInRag: boolean | undefined
): DocumentInfo => ({ documentId, includeInRag }) as DocumentInfo;
...
test('document diff bounds work for very long rewrites while preserving common edges', () => {
	...
	const after = [
		'Same start',
		...Array.from({ length: 300 }, (_, index) => `New ${index}`),
		'Same end',
	];

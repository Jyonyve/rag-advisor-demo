...
import type { DocumentInfo } from '@rag-advisor-demo/shared/domain';
import { DEFAULT_CHAT_MODEL, DEFAULT_EMOTION, LANG_KEYS } from '@rag-advisor-demo/shared/config';
...
type EditorState = Pick<
	DocumentInfo,
	'title' | 'body' | 'documentKind' | 'issuer' | 'viewpoint' | 'includeInRag'
>;
...
const toEditorState = (document: DocumentInfo): EditorState => ({
	...
	includeInRag: document.includeInRag ?? document.retrievalEnabled,
});
...
const isEditorDirty = (document: DocumentInfo, editor: EditorState): boolean => {
	...
	return (
		...
		original.includeInRag !== editor.includeInRag
	);
};
...
export function DocumentPage() {
	...
	const filteredDocuments = useMemo(
		() => filterDocumentsByRagPreference(documents, ragFilter),
		[documents, ragFilter]
	);
	...
	useEffect(() => {
		...
		setHeaderInfo({
			...
			mobileImageUrl:
				getImageForEmotion(characterRes.characterPortraits[character.characterId], DEFAULT_EMOTION) ??
				'',
		});
	...
	const lastRequestText = resolveLastDocumentRequest(
		selected?.documentId,
	...
	const setRetrievalPreference = async (enabled: boolean) => {
		...
		const previousPreference = editor.includeInRag;
		const updatedEditor = { ...editor, includeInRag: enabled };
		...
		try {
			...
			setEditor({ ...editor, includeInRag: previousPreference });
		}
	};
	...
	return (
		<>
			...
			>
				...
				<GlassCard
					sx={{ mb: 2, height: 'auto' }}
					contentProps={{ sx: { p: 2, '&:last-child': { pb: 2 } } }}
				>
				...
				<Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems="stretch">
						...
						<Box sx={{ display: 'flex', alignItems: 'stretch' }}>
							...
							>
								...
								<Tab
									disableRipple
									value="included"
									label={getLangText(LANG_KEYS.DOCUMENT_FILTER_INCLUDED)}
								/>
								<Tab
									disableRipple
									value="notIncluded"
									label={getLangText(LANG_KEYS.DOCUMENT_FILTER_NOT_INCLUDED)}
								/>
							</Tabs>
							<Tooltip title={getLangText(LANG_KEYS.NEW_DOCUMENT)}>
								...
								>
									<IconButton onClick={createDraft} disabled={!sessionId || api.isMutating}>
										<AddIcon />
									</IconButton>
								</Box>
							</Tooltip>
					...
					<Box sx={{ flex: 1, minWidth: 0 }}>
						{selected && editor ? (
							<GlassCard variant="outlined" sx={{ height: 'auto' }}>
								<Stack spacing={2}>
									...
									{comparison?.documentId === selected.documentId && isComparisonVisible && (
										<DocumentDiff
											before={editorToDiffText(comparison.before)}
											after={editorToDiffText(editor)}
										/>
									)}
									{selected.status === 'draft' && (
										<GlassPaper sx={{ p: 2 }}>
									...
									<Stack direction="row" spacing={1} justifyContent="flex-end" flexWrap="wrap">
										{selected.status === 'draft' && (
											<>
												<GlassButton startIcon={<DeleteOutlineIcon />} onClick={deleteDraft}>
							...
							<GlassCard
								sx={{ height: 'auto' }}
								contentProps={{ sx: { p: 3, '&:last-child': { pb: 3 } } }}
							>
								<Typography color="text.secondary">
									{getLangText(LANG_KEYS.SELECT_OR_CREATE_DOCUMENT)}
								</Typography>
							</GlassCard>
						)}
					</Box>

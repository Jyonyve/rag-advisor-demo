...
import demoRoutes from './route/demo.routes.js';
...
import { ApiErrorResponse } from '@rag-advisor-demo/shared/api';
import { APPNAME, MODULE_NAMES } from '@rag-advisor-demo/shared/config';
...
async function createServer() {
						...
						signUpPOST: async function (input) {
							...
							if (response.status === 'OK') {
								try {
									...
									flowLogger.info('server', 'auth.userSync.complete', { userId: response.user.id });
								} catch (error) {
									flowLogger.error('server', 'auth.userSync.failed', {
	...
	app.use(
		'/assets',
		express.static(ensureLocalImageStorageRoot(), { fallthrough: true, maxAge: '1h' })
	);
	...
	app.use(`${BASE_API}/${MODULE_NAMES.DEMO}`, demoRoutes);

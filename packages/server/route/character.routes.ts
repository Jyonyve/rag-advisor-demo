...
import { CharacterInfo } from '@rag-advisor-demo/shared/domain';
import {
...
router.post(
	...
	asyncHandler(async (req: Request, res: Response): Promise<void> => {
		...
		const fullUploadPath = getCharacterImageStorageDir(characterId);

		try {
			if (!fs.existsSync(fullUploadPath)) {
				fs.mkdirSync(fullUploadPath, { recursive: true });
				flowLogger.info('character.routes', 'characterFolder.created', { characterId });
			}

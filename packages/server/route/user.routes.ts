...
import { UserResponse } from '@rag-advisor-demo/shared/api';
import { RUNTIME_USER_IMAGE_DIR } from '@rag-advisor-demo/shared/config';
import { ApiError, UserInfo } from '@rag-advisor-demo/shared/domain';
import { flowLogger, serializeError } from '../util/jsonlLogger.js';
import { getUserImageStorageDir } from '../util/imageStorageUtils.js';
...
router.delete(
	...
	asyncHandler(async (req: Request, res: Response): Promise<void> => {
		...
		try {
			...
			const filePath = path.join(getUserImageStorageDir(userId), fileName);

			if (fs.existsSync(filePath)) {
				fs.unlinkSync(filePath);
				flowLogger.info('user.routes', 'avatar.deleted', { userId, fileName });
			}
...
router.post(
	...
	asyncHandler(async (req: Request, res: Response): Promise<void> => {
		...
		const fullUploadPath = getUserImageStorageDir(userId);

		try {
			if (!fs.existsSync(fullUploadPath)) {
				fs.mkdirSync(fullUploadPath, { recursive: true });
				flowLogger.info('user.routes', 'userFolder.created', { userId });
			}

...
test('RAG tracing requires both development mode and the explicit flag', () => {
	assert.equal(isRagTraceEnabled({ NODE_ENV: 'development', RAG_ADVISOR_RAG_TRACE: 'true' }), true);
	assert.equal(
		isRagTraceEnabled({ NODE_ENV: 'development', RAG_ADVISOR_RAG_TRACE: 'false' }),
		false
	);
	assert.equal(isRagTraceEnabled({ NODE_ENV: 'production', RAG_ADVISOR_RAG_TRACE: 'true' }), false);
	assert.equal(isRagTraceEnabled({ NODE_ENV: 'test', RAG_ADVISOR_RAG_TRACE: 'true' }), false);
});
...
test('RAG trace emits context and strips retrieved document content', () => {
	...
	try {
		traceRagEvent(
			...
			{ NODE_ENV: 'development', RAG_ADVISOR_RAG_TRACE: 'true' }
		);
...
test('RAG trace emits nothing when disabled', () => {
	...
	try {
		traceRagEvent(
			...
			'disabled',
			{},
			{ NODE_ENV: 'development', RAG_ADVISOR_RAG_TRACE: 'false' }
		);

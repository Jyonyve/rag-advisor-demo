...
import { processCharacterImagePair } from './imageProcessingUtils.js';
import { getCharacterImageStorageDir } from './imageStorageUtils.js';

const storageRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'rag-advisor-demo-image-crop-'));
...
const assertPairDimensions = async (characterId: string): Promise<void> => {
	...
	const portrait = await sharp(path.join(directory, `${characterId}_0.avif`)).metadata();
	const avatar = await sharp(path.join(directory, `${characterId}_0_a.avif`)).metadata();
...
test('character image processing stores only the cropped pair and preserves it after decode failure', async () => {
	...
	const directory = getCharacterImageStorageDir(characterId);
	const portraitPath = path.join(directory, `${characterId}_0.avif`);
	const avatarPath = path.join(directory, `${characterId}_0_a.avif`);
	...
	assert.deepEqual(filesBefore, [`${characterId}_0.avif`, `${characterId}_0_a.avif`]);

...
export const isRagTraceEnabled = (environment: RagTraceEnvironment = process.env): boolean =>
	(environment.NODE_ENV ?? 'development') === 'development' &&
	environment.RAG_ADVISOR_RAG_TRACE === 'true';
...
export const createRagTraceContext = (
	context: Omit<RagTraceContext, 'traceId'>
): RagTraceContext => ({ traceId: randomUUID(), ...context });

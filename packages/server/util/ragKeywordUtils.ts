...
export const extractQueryBoostTerms = (
	...
	for (const value of values) {
		...
		for (const token of tokens) {
			...
			if (
				stripped.length < 2 ||
				queryKeywordStopWords.has(normalized) ||
				queryKeywordStopWords.has(stripped)
			) {
				continue;
			}
...
export const extractRetrievalBoostTerms = (
	...
	const canonicalize = (term: string): string =>
		stripKoreanPredicateEnding(stripKoreanParticle(term.toLowerCase()));
	const terms = new Set(
		extractQueryBoostTerms([userQuery, criticalTerm ?? ''], aliases)
			.map(canonicalize)
			.filter(
				(term) => term.length >= 2 && !excludedTerms.has(term) && !retrievalQuestionStopWords.has(term)
			)
	);
...
export const selectEarliestRelevantMatches = <T>(
	...
	return scoredItems
		...
		.sort(
			(left, right) =>
				getSequence(left.item) - getSequence(right.item) ||

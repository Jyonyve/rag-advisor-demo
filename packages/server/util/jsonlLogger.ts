...
class SingleFlowLogger {
	...
	private constructor() {
		...
		this.filePath = path.resolve(process.cwd(), 'logs', 'rag-advisor-demo-flow.jsonl');
	...
	private isDisabled(): boolean {
		return process.env.NODE_ENV === 'test' || process.env.RAG_ADVISOR_DEMO_DISABLE_JSONL_LOGS === '1';
	}
...
export const createOperationLogger = (
	...
	return {
		...
		checkpoint: (stage, data) =>
			flowLogger.info(module, 'checkpoint', withContext({ stage, ...data })),

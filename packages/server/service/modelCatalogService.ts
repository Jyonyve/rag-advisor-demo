import { ModelCatalogEntry, ModelCatalogResponse } from '@rag-advisor-demo/shared/api';
import { MODEL_LIMITS_INFO, SELECTABLE_MODEL_INFO } from '@rag-advisor-demo/shared/config';
import { AiModelInfo, ApiError } from '@rag-advisor-demo/shared/domain';
...
export const buildOpenRouterCatalogEntries = (input: unknown): ModelCatalogEntry[] => {
	...
	const candidates = parsed.data
		...
		.sort(
			(left, right) =>
				right.model.created - left.model.created || left.model.id.localeCompare(right.model.id)
		);
...
let cachedLiveEntries:
	| { entries: ModelCatalogEntry[]; expiresAt: number; refreshedAt: string }
	| undefined;

...
Infer a fitting document kind, issuer, and viewpoint when the request omits them.
...
Use the user's language unless the requested artifact clearly requires another language.`,
		...
		return documentStore.createDraft({
			...
			promptVersion: 'in-world-document-v1',
		});
	},
...
Use the current draft as the base. Preserve its title, document kind, issuer, and viewpoint unless the edit instruction asks to change them or the current metadata is clearly incomplete.
Treat official lore and direct conversation evidence as constraints. Treat approved in-world documents as viewpoint-bound claims, not objective truth.

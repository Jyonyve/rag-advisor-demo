...
import { getPublicDemoModel, isDemoGuest } from './demoAccessService.js';
...
export const financeReportService = {
	generateDraft: async (
		input: FinanceReportDraftCreate,
		userId: string,
		session: SessionInfo,
		...
		const [characterResponse, chatResponse, profileResponse, aiModelInfo, characterLoreResponse] =
			await Promise.all([
				...
				demoGuest
					? Promise.resolve(getPublicDemoModel('report'))
	...
	generateFallbackDraft: async (
		input: FinanceReportDraftCreate,
		userId: string,
		session: SessionInfo,
		reason: DemoUsageReason
	) => {
		const [characterResponse, chatResponse, profileResponse, loreResponse, documents] =
			await Promise.all([
				characterStore.getCharacter(session.characterId),
				chatStore.getAllChatTurns(session.sessionId),
				profileStore.getProfileBySessionId(session.sessionId),
				loreStore.getLoresByCharacter(session.characterId, userId),
				documentStore.getDocumentsBySession(session.sessionId, userId),
			]);
		const character = characterResponse.characterInfo;
		if (character.domain !== 'finance') {
			throw new ApiError(400, 'Finance reports require the Finance Character.');
		...
		const context = resolveRagContext({
			...
			profile: profileResponse.profileInfo,
			memories: {
				langCode: detectLanguage(input.requestText),
		...
		const eligible = [...getEligibleProducts(context).values()].slice(0, 3);
		const exclusions = context.evidence.excluded.map(
			(item) => `- ${item.sourceKind}: ${item.reason} (${item.count})`
		);
		const body = `# Deterministic fallback finance report

> Live generation is unavailable (${reason}). This report is deterministic approved demo content and was not generated by a provider.

## Current request (temporary condition)

${context.currentMessage}

## Fixed canonical session profile

\`\`\`json
${JSON.stringify(context.sessionProfile, null, 2)}
\`\`\`

## Retrieved memories

${recentTurns.map((turn) => `- Turn ${turn.sequence} [${turn.chatTurnId}]`).join('\n') || '- None'}

## Eligible approved fictional product evidence

${eligible.map((item) => `- ${item.title} [${item.loreId}]`).join('\n') || '- None eligible'}

## Approved session documents

${(context.memories.relevantDocuments ?? []).map((item) => `- ${item.title} [${item.documentId}]`).join('\n') || '- None'}

## Exclusions

${exclusions.join('\n') || '- None'}

## Bounded sample conclusion

${
	eligible.length
		? 'The eligible fictional options above can be compared against the fixed profile, liquidity need, horizon, and risk preference. No live personalized recommendation was generated.'
		: 'The approved fixtures do not support a product-specific recommendation for this request.'
}

Demo notice: Products and scenarios are fictional demo data. This is educational information, not financial advice or legal advice.`;
		return documentStore.createDraft({
			userId,
			sessionId: session.sessionId,
			characterId: session.characterId,
			origin: 'generated',
			includeInRag: false,
			title: 'Deterministic fallback finance report',
			body,
			documentKind: 'personalized-finance-report',
			issuer: 'Fictional Finance RAG Demo',
			viewpoint: 'Educational deterministic fallback',
			groundingMode: 'grounded',
			requestText: input.requestText,
			sourceRefs: buildFinanceReportSourceRefs(
				context.memories,
				recentTurns.map(({ chatTurnId }) => chatTurnId)
			),
			modelName: getPublicDemoModel('report').model,
			promptVersion: 'finance-report-fallback-v1',
		});
	},
};

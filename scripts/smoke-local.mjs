const baseUrl = process.env.RAG_ADVISOR_DEMO_LOCAL_BASE_URL || 'http://localhost:3000';
const smokeEmail = process.env.RAG_ADVISOR_DEMO_SMOKE_EMAIL;
const smokePassword = process.env.RAG_ADVISOR_DEMO_SMOKE_PASSWORD;
...
const checkAnonymousAuthProtection = async () => {
	...
	assert(
		response.status === 401,
		`/api/user/get-me should reject anonymous requests with 401, got ${response.status}`
	);
};
...
const main = async () => {
	...
	assert(html.includes('<title>RAG Advisor Demo</title>'), 'SSR HTML did not include the app title');
	...
	if (requireAuthSmoke && (!smokeEmail || !smokePassword)) {
		throw new Error(
			'Authenticated smoke requires RAG_ADVISOR_DEMO_SMOKE_EMAIL and RAG_ADVISOR_DEMO_SMOKE_PASSWORD in the shell environment.'
		);
	}
